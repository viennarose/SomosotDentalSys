...
class Index extends Component
{
    ...
    public function loadAppointments(){
        ...
        $query = Appointment::orderBy('schedule', 'asc')
            ->search($this->search);
    ...
    public function mailConcern(int $appointment_id){
        try{
            $appointment = Appointment::findOrFail($appointment_id);
            Mail::to("$appointment->email")->send(new concernMail($appointment));
            return redirect('admin/appointments')->with('message', 'Email has been sent to' .$appointment->email);
        }catch(\Exception $e){
            return redirect('admin/appointments')->with('message', 'Something went wrong.');
        }
