                ...
                <div class="row">
                    ...
                    <div class="col-md-6 form-control border-0">
                        <div class="form-floating mb-4">
                            <select class="form-select" wire:model="services_id" name="services_id">
                                @foreach($services as $service)
                                    @if($service->id==$this->serviceId)
                                        <option value="{{$service->id}}" selected="selected">{{$this->service->service_type}}</option>
                                    @endif
                                @endforeach
                                @foreach($services as $service)
                                    @if($service->status==0 && $service->id!=$this->serviceId)
